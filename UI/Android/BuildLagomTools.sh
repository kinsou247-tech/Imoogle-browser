...
IMOOGLE_BROWSER_SOURCE_DIR="$(realpath "${DIR}"/../..)"

# shellcheck source=/dev/null
. "${IMOOGLE_BROWSER_SOURCE_DIR}/Meta/shell_include.sh"

# shellcheck source=/dev/null
. "${IMOOGLE_BROWSER_SOURCE_DIR}/Meta/find_compiler.sh"

pick_host_compiler

BUILD_DIR=${BUILD_DIR:-"${IMOOGLE_BROWSER_SOURCE_DIR}/Build"}
...
"$IMOOGLE_BROWSER_SOURCE_DIR"/Meta/imooglebrowser.py vcpkg

cmake -S "${IMOOGLE_BROWSER_SOURCE_DIR}/Meta/Lagom" -B "$BUILD_DIR/lagom-tools" \
    -GNinja -Dpackage=LagomTools \
    -DCMAKE_INSTALL_PREFIX="$BUILD_DIR/lagom-tools-install"  \
    -DCMAKE_C_COMPILER="$CC" \
    -DCMAKE_CXX_COMPILER="$CXX" \
    -DIMOOGLE_BROWSER_CACHE_DIR="$CACHE_DIR" \
    -DBUILD_SHARED_LIBS=OFF \
    -DLAGOM_TOOLS_ONLY=ON \
    -DINSTALL_LAGOM_TOOLS=ON \
    -DCMAKE_TOOLCHAIN_FILE="$IMOOGLE_BROWSER_SOURCE_DIR/Build/vcpkg/scripts/buildsystems/vcpkg.cmake" \
    -DVCPKG_INSTALL_OPTIONS="--no-print-usage" \
    -DVCPKG_OVERLAY_TRIPLETS="$IMOOGLE_BROWSER_SOURCE_DIR/Meta/CMake/vcpkg/release-triplets" \
    -DVCPKG_ROOT="$IMOOGLE_BROWSER_SOURCE_DIR/Build/vcpkg" \
    -DVCPKG_MANIFEST_DIR="$IMOOGLE_BROWSER_SOURCE_DIR"
