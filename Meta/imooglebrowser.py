...
def main():
    ...
    parser = argparse.ArgumentParser(description="ImoogleBrowser")
    ...
    if platform.host_system != HostSystem.Windows and os.geteuid() == 0:
        print("Do not run imooglebrowser.py as root, your Build directory will become root-owned", file=sys.stderr)
        sys.exit(1)
    elif platform.host_system == HostSystem.Windows and "VCINSTALLDIR" not in os.environ:
        print("imooglebrowser.py must be run from a Visual Studio enabled environment", file=sys.stderr)
        sys.exit(1)

    if "target" in args:
        if args.target == "imooglebrowser":
            args.target = "ImoogleBrowser"
...
def configure_main(platform: Platform, preset: str, cc: str, cxx: str) -> Path:
    imooglebrowser_source_dir, build_preset_dir = configure_build_env(platform, preset)
    build_vcpkg()

    if build_preset_dir.joinpath("build.ninja").exists() or build_preset_dir.joinpath("imooglebrowser.sln").exists():
    ...
    if "Swift" in preset:
        compilers = pick_swift_compilers(platform, imooglebrowser_source_dir)
    ...
    # https://github.com/ImoogleBrowserBrowser/imooglebrowser/blob/master/Documentation/BuildInstructionsImoogleBrowser.md#unable-to-find-a-build-program-corresponding-to-ninja
...
def configure_skia_jemalloc() -> list[str]:
    ...
    # https://github.com/ImoogleBrowserBrowser/imooglebrowser/issues/261
...
def configure_build_env(platform: Platform, preset: str) -> tuple[Path, Path]:
    imooglebrowser_source_dir = ensure_imooglebrowser_source_dir()
    build_root_dir = imooglebrowser_source_dir / "Build"
    ...
    os.environ["PATH"] += os.pathsep + str(imooglebrowser_source_dir.joinpath("Toolchain", "Local", "cmake", "bin"))
    ...
    return imooglebrowser_source_dir, build_preset_dir
...
def ensure_imooglebrowser_source_dir() -> Path:
    imooglebrowser_source_dir = os.environ.get("LADYBIRD_SOURCE_DIR", None)
    imooglebrowser_source_dir = Path(imooglebrowser_source_dir) if imooglebrowser_source_dir else None

    if not imooglebrowser_source_dir or not imooglebrowser_source_dir.is_dir():
        ...
        imooglebrowser_source_dir = Path(root_dir)

    return imooglebrowser_source_dir
...
def run_main(host_system: HostSystem, build_dir: Path, target: str, args: list[str]):
    ...
    if host_system == HostSystem.macOS and target in (
        ...
        run_args.append(str(build_dir.joinpath("bin", "ImoogleBrowser.app", "Contents", "MacOS", target)))
    else:
...
def debug_main(host_system: HostSystem, build_dir: Path, target: str, debugger: str, debugger_commands: list[str]):
    ...
    if target == "ImoogleBrowser" and host_system == HostSystem.macOS:
        gdb_args.append(str(build_dir.joinpath("bin", "ImoogleBrowser.app")))
    else:
...
def profile_main(host_system: HostSystem, build_dir: Path, target: str, args: list[str]):
    ...
    if target == "ImoogleBrowser" and host_system == HostSystem.macOS:
        valgrind_args.append(str(build_dir.joinpath("bin", "ImoogleBrowser.app")))
    else:
...
def clean_main(platform: Platform, preset: str):
    ...
    user_vars_cmake_module = imooglebrowser_source_dir.joinpath("Meta", "CMake", "vcpkg", "user-variables.cmake")
