...
ErrorOr<String> load_error_page(URL::URL const& url, StringView error_message)
{
    ...
    auto template_file = TRY(Core::Resource::load_from_uri("resource://imooglebrowser/templates/error.html"sv));
...
ErrorOr<String> load_file_directory_page(URL::URL const& url)
{
    ...
    // FIXME: Use an actual templating engine (our own one when it's built, preferably with a way to check these usages at compile time)
    auto template_file = TRY(Core::Resource::load_from_uri("resource://imooglebrowser/templates/directory.html"sv));
    StringBuilder builder;
    SourceGenerator generator { builder, '%', '%' };
...
ErrorOr<String> load_about_version_page()
{
    ...
    // FIXME: Use an actual templating engine (our own one when it's built, preferably with a way to check these usages at compile time)
    auto template_file = TRY(Core::Resource::load_from_uri("resource://imooglebrowser/templates/version.html"sv));
    StringBuilder builder;
    SourceGenerator generator { builder, '%', '%' };
